...
async function addCoins(amount) {
  ...
  // All coin changes go through the atomic Supabase RPC.
  // Never trust/update the client balance directly: RLS may reject it and the UI
  // can otherwise show coins that were never saved.
  const { data, error } = await sb.rpc('change_kolecoins', { p_amount: delta });

  if (error) {
    console.error('change_kolecoins failed:', error);
    const msg = String(error.message || '').toLowerCase();
    if (msg.includes('function') && msg.includes('does not exist')) {
      showToast('بخش پاداش دیتابیس نصب نشده؛ fix-rewards.sql رو اجرا کن');
    } else if (msg.includes('not enough')) {
      showToast('KoleCoin کافی نداری');
    } else {
  ...
  // Re-read the real row from Supabase so the displayed balance is exactly
  // what is stored in the database.
  const serverBalance = Number(data);
  if (Number.isFinite(serverBalance)) state.profile.coins = serverBalance;
  await refreshProfile();
  updateDashboardUI();

  if (typeof addXP === 'function' && delta > 0) {
    await addXP(Math.min(delta, 120));
  }
  return true;
}
